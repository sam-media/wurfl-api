...
class CentralTestAssertion
{
    ...
    {
        switch (self::assertion_type) {
            case self::TYPE_DEVICE:
                $this->result = self::assertDevice($device, $response);
                break;
            case self::TYPE_CAPABILITY:
                $this->result = self::assertCapability($device, $response);
                break;
            case self::TYPE_VIRTUAL_CAPABILITY:
                $this->result = self::assertVirtualCapability($device, $response);
                break;
            default:
    ...
    public function assertDevice(WURFL_CustomDevice $device, $response = null)
    {
        ...
        try {
            ...
            throw new Exception(
                "The expected WURFL ID ($this->expected_id) is not in the loaded WURFL Data!  Detected as: {$device->id}",
                null,
                $e
            );
        }
    ...
    {
        $actual = $response
            ? $response['capabilities'][$this->expected_id]
