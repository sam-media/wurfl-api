...
class CentralTest
{
    ...
    public function run(WURFL_WURFLManager $wurflManager)
    {
        ...
        if ($this->min_version && version_compare(WURFL_Constants::API_VERSION, $this->min_version, '<')) {
            throw new CentralTest_InvalidIntrospectorVersionException(
                "Test requires WURFL PHP API >= $this->min_version (current $wurflManager->release_version)"
            );
        }
        ...
        if ($this->mode != self::MODE_ALL) {
            $required_mode = ($this->mode == self::MODE_HIGH_PERFORMANCE)
                ? WURFL_Configuration_Config::MATCH_MODE_PERFORMANCE : WURFL_Configuration_Config::MATCH_MODE_ACCURACY;

            WURFL_Configuration_ConfigHolder::getWURFLConfig()
                ->matchMode($required_mode);
        }
        ...
        foreach (self::assertions as $assertion) {
...
class CentralTest_InvalidIntrospectorModeException
    extends Exception
{
}
...
class CentralTest_InvalidIntrospectorVersionException
    extends Exception
{
}
