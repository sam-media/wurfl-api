...
class WURFL_VirtualCapability_UserAgentTool
{
    ...
    protected function assignProperties($device)
    {
        ...
        if ($device->os->setContains($device->device_ua, 'Windows CE',
                'Windows Mobile') && $device->browser->set('IE Mobile')
        ) {
            return $device;
        }
        ...
        if (strpos($device->device_ua, 'Windows Phone') !== false || strpos($device->device_ua, '; wds') !== false) {
            if ($device->os->setRegex($device->device_ua, '/Windows Phone(?: OS)? ([0-9]+\.[0-9])/', 'Windows Phone',
                    1) || $device->os->setRegex($device->device_ua,
                    '#UCWEB/\d\.\d \(Windows;.+?; wds ?([\d\.]+?);.+UCBrowser#', 'Windows Phone', 1)
            ) {
        ...
        if (strpos($device->device_ua, 'Android') !== false || strpos($device->device_ua, ' Adr ') !== false) {
            ...
            //Is FB app?
            if ($device->browser->setRegex($device->browser_ua, '/^Mozilla\/[45]\.0.+?Android.+?AppleWebKit.+FBAN/',
                'FaceBook Android App', $device->os->version)
            ) {
                return $device;
            }
            ...
            //Is 360Browser?
            if (strpos($device->browser_ua, 'Aphone Browser') !== false || strpos($device->browser_ua,
                    '360browser') !== false
            ) {
            ...
            //Is UA Fennec?
            if ($device->browser->setRegex($device->browser_ua, '/(?:Firefox|Fennec)\/([0-9]?[0-9]\.[0-9]?)/',
                'Firefox Mobile', 1)
            ) {
                return $device;
            }
            ...
            //Is UA Opera Mobi?
            if ($device->browser->setRegex($device->browser_ua, '/Opera Mobi\/.*Version\/([0-9]?[0-9])/',
                'Opera Mobile', 1)
            ) {
                return $device;
            }
            ...
            //Is UA Opera Tablet?
            if ($device->browser->setRegex($device->browser_ua, '/Opera Tablet\/.*Version\/([0-9]?[0-9])/',
                'Opera Tablet', 1)
            ) {
                return $device;
            }
            ...
            if ($device->browser->setRegex($device->browser_ua, '/Silk\/([0-9]\.[0-9]).+?Silk\-Accelerated/',
                'Amazon Silk Browser', 1)
            ) {
                return $device;
            }
            ...
            if ($device->browser->setRegex($device->browser_ua, '#SamsungBrowser/([\d\.]+) Chrome/[\d\.]+#',
                'Samsung Browser', 1)
            ) {
                return $device;
            }
            ...
            //Is UA Chromium?
            if ($device->browser->setRegex($device->browser_ua, '/Version\/.+?Chrome\/([0-9]?[0-9])\.?/', 'Chromium',
                1)
            ) {
                return $device;
            }
            ...
            if ($device->browser->setRegex($device->browser_ua, '/Version\/\d/', 'Android Webkit',
                $device->os->version)
            ) {
                return $device;
            }
        ...
        if (strpos($device->device_ua, 'Silk') !== false && $device->browser->setRegex($device->browser_ua,
        ...
        //Is UA iOS?
        if (strpos($device->device_ua_normalized, 'iPhone') !== false || strpos($device->device_ua_normalized,
                'iPad') !== false || strpos($device->device_ua_normalized,
                'iPod') !== false || strpos($device->device_ua_normalized, '(iOS;') !== false
        ) {
            $device->os->name = 'iOS';

            if ($device->os->setRegex($device->device_ua_normalized,
                    '/Mozilla\/[45]\.[0-9] \((iPhone|iPod|iPad);(?: U;)? CPU(?: iPhone|) OS ([0-9]_[0-9](?:_[0-9])?) like Mac OS X/',
                    'iOS', 2)
                || $device->os->setRegex($device->device_ua_normalized,
                    '#^[^/]+?/[\d\.]+? \(i[A-Za-z]+; iOS ([\d\.]+); Scale/[\d\.]+\)#', 'iOS', 1)
                || $device->os->setRegex($device->device_ua_normalized, '#^server-bag \[iPhone OS,([\d\.]+),#', 'iOS',
                    1)
                || $device->os->setRegex($device->device_ua_normalized, '#^i(?:Phone|Pad|Pod)\d+?,\d+?/([\d\.]+)#',
                    'iOS', 1)
            ) {
            ...
            if ($device->os->setRegex($device->device_ua, '#UCWEB/[\d\.]+ \(iOS;.+?OS ([\d_]+);.+UCBrowser/#', 'iOS',
                1)
            ) {
                $device->os->version = str_replace('_', '.', $device->os->version);
            }
            ...
            if ($device->browser->setRegex($device->browser_ua,
            ...
            if ($device->browser->setRegex($device->browser_ua,
                '/^Mozilla\/[45]\.0.+?like Mac OS X.+?FxiOS\/([0-9]+?)\.[0-9].+?Mobile\/[0-9A-Za-z]+ Safari\/[0-9A-Za-z]+\./',
                'Firefox on iOS', 1)
            ) {
                return $device;
            }
            ...
            if ($device->browser->setRegex($device->browser_ua, '#UCWEB/\d\.\d \(iOS;.+?OS [\d_]+;.+UCBrowser/(\d+)#',
                'UC Web Browser on iOS', 1)
            ) {
                return $device;
            }
            ...
            if ($device->browser->setRegex($device->browser_ua,
                '/^Mozilla\/[45]\.0.+?like Mac OS X.+?AppleWebKit.+?Mobile\/[0-9A-Za-z]+.*FBAN/', 'FaceBook on iOS',
                $device->os->version)
            ) {
                return $device;
            }
            ...
            // Is UA iOS Safari?
            if ($device->browser->setRegex($device->browser_ua, '#^Mozilla.+like Mac OS X.+Version/([\d\.]+)#',
                'Mobile Safari', 1)
            ) {
                return $device;
            }
        ...
        if (strpos($device->device_ua, 'OviBrowser') !== false && $device->browser->setRegex($device->browser_ua,
                '/\bS40OviBrowser\/([0-9]\.[0-9])/', 'S40 Ovi Browser', 1) && $device->os->set('Nokia Series 40')
        ) {
            return $device;
        }

        //Is Series60?
        if ($device->os->setRegex($device->device_ua, '#(?:SymbianOS|Series60|S60)/(\d+(?:\.\d+)?)#', 'Symbian S60',
                1) || $device->os->setRegex($device->device_ua, '#UCWEB/\d\.\d \(Symbian;.+?S60 V(\d+)#', 'Symbian S60',
                1)
        ) {
            ...
            if ($device->browser->setRegex($device->browser_ua, '/NokiaBrowser\/([0-9]\.[0-9])/', 'Symbian S60 Browser',
                1)
            ) {
                return $device;
            }

            if ($device->browser->setRegex($device->browser_ua, '/Opera Mobi.+Version\/([0-9]?[0-9]\.[0-9]?[0-9])/',
                'Opera Mobi', 1)
            ) {
                return $device;
            }

            if ($device->browser->setRegex($device->browser_ua, '#UCWEB/\d\.\d \(Symbian;.+?UCBrowser/(\d+)#',
        ...
        if (strpos($device->device_ua, 'BlackBerry') !== false || strpos($device->device_ua, '(BB10; ') !== false) {
            ...
            if ($device->os->setRegex($device->device_ua,
        ...
        if (strpos($device->device_ua, 'RIM Tablet OS') !== false
            && $device->os->setRegex($device->device_ua, '/RIM Tablet OS ([0-9]\.[0-9]).+?Version\/([0-9]\.[0-9])/',
                'RIM Tablet OS', 1)
        ) {
        ...
        if (strpos($device->device_ua, 'Dolfin') !== false
            && $device->os->setRegex($device->device_ua, '/SAMSUNG.+?\bBada\/([0-9]\.[0-9]);?.+Dolfin\/([0-9]\.[0-9])/',
                'Bada', 1)
        ) {
        ...
        if (strpos($device->device_ua, 'Dolfin') !== false
            && $device->browser->setRegex($device->browser_ua, '/UP\.(?:Browser|Link)\/([0-9]\.[0-9])/',
                'Openwave Browser', 1)
        ) {
            return $device;
        }
        ...
        //Is UA webOS?
        if ($device->os->setRegex($device->device_ua, '/^Mozilla\/[45]\.0 \((?:Linux; )?webOS\/([0-9]\.[0-9])/',
            'webOS', 1)
        ) {
        ...
        if (strpos($device->device_ua, 'Opera') !== false) {
            ...
            if ($device->browser->setContains($device->device_ua, 'Opera Mobi', 'Opera Mobile')) {
                if ($device->browser->setRegex($device->device_ua, '/Opera Mobi.+Version\/([0-9]?[0-9]\.[0-9]?[0-9])/',
                    null, 1)
                ) {
                    return $device;
                }
            ...
            //Is UA Opera Sync?
            if ($device->browser->setRegex($device->device_ua,
        ...
        if (strpos($device->device_ua, 'Java') !== false && strpos($device->device_ua, 'UCBrowser/') !== false) {
            if ($device->browser->setRegex($device->browser_ua, '#UCWEB/\d\.\d \(Java;.+?UCBrowser/(\d+)#',
                'UCBrowser Java Applet', 1)
            ) {
                return $device;
            }
        }
        ...
        //Final ditch effort
        if ($device->browser->setRegex($device->browser_ua, '/(?:MIDP.+?CLDC)|(?:UNTRUSTED)|(?:MIDP-2.0)/',
            'Java Applet')
        ) {
            return $device;
        }
        ...
        //Baidu browser?
        if ($device->os->setRegex($device->device_ua,
            '/^Mozilla\/[0-9]\.0 .+?((?:Windows|Linux|PPC|Intel) [a-zA-Z0-9 _\.\-]+).+bdbrowser(?:_i18n)?\/(\d+)/', 1)
        ) {
        ...
        //360 Browser
        if ((strpos($device->device_ua, '360Browser') !== false || strpos($device->device_ua,
                    ' 360SE') !== false) && $device->os->setRegex($device->device_ua,
                '/^Mozilla\/[0-9]\.0 .+?((?:Windows|Linux|PPC|Intel) [a-zA-Z0-9 _\.\-]+).+(?:360Browser|360SE)/', 1)
        ) {
            $device->browser->set('360 Browser', null);
        ...
        if (strpos($device->device_ua, 'Trident') !== false || strpos($device->device_ua, ' Edge/') !== false) {
            ...
            if ($device->os->setRegex($device->device_ua,
                    ...
                    1) || $device->os->setRegex($device->device_ua,
                    '#^Mozilla/[45]\.0 \((Windows NT [\d\.]+).+? Edge/([\d\.]+)#', 1)
            ) {
        ...
        if (strpos($device->device_ua, 'YaBrowser') !== false
            && $device->os->setRegex($device->device_ua,
        ...
        if (strpos($device->device_ua, 'OPR') !== false
            && $device->os->setRegex($device->device_ua,
                '/^Mozilla\/[0-9]\.0 .+?((?:Windows|Linux|PPC|Intel) [a-zA-Z0-9 _\.\-]+).+Chrome\/.+OPR\/([\d\.]+)/', 1)
        ) {
        ...
        if (strpos($device->device_ua, 'Opera') !== false
            ...
            $device->browser->setRegex($device->browser_ua, '/^Opera\/.+? Version\/([0-9]?[0-9]\.[0-9][0-9]?)/', null,
                1);
        ...
        if (strpos($device->device_ua, 'Chrome') !== false) {
            //Chrome Mac
            if ($device->os->setRegex($device->device_ua,
                '/^Mozilla\/[0-9]\.0 \(Macintosh;(?: U;)?([a-zA-Z_ \.0-9]+)(?:;)?.+? Chrome\/([\d\.]+)\.?/', 1)
            ) {
            ...
            //Chrome
            if ($device->os->setRegex($device->device_ua,
                '/^Mozilla\/[0-9]\.0 \((?:Windows;|X11;)?(?: U; )?([a-zA-Z_ \.0-9]+)(?:;)?.+? Chrome\/([\d\.]+)\.?/', 1)
            ) {
                $device->browser->set('Chrome', $device->os->getLastRegexMatch(2));
        ...
        if (strpos($device->device_ua_normalized, 'Safari') !== false
            && $device->os->setRegex($device->device_ua_normalized,
                '/Mozilla\/[0-9]\.0 \((?:(?:Windows|Macintosh); (?:U; |WOW64; )?)?([a-zA-Z_ \.0-9]+)(?:;)?.+? Version\/([\d\.]+)\.?/',
                1)
        ) {
        ...
        if (strpos($device->device_ua, 'Firefox') !== false) {
            //Firefox - Windows
            if ($device->os->setRegex($device->device_ua,
                '/^Mozilla\/[0-9]\.0 .+(Windows [0-9A-Za-z \.]+;).+?rv:.+?Firefox\/([\d\.]+)/', 1)
            ) {
            ...
            //Firefox
            if ($device->os->setRegex($device->device_ua,
                '/^Mozilla\/[0-9]\.0 \((?:X11|Macintosh); (?:U; |Ubuntu; |)((?:Intel|PPC|Linux) [a-zA-Z0-9\- \._\(\)]+);.+?rv:.+?Firefox\/([\d\.]+)/',
                1)
            ) {
                $device->browser->set('Firefox', $device->os->getLastRegexMatch(2));
        ...
        if (strpos($device->browser_ua, 'CFNetwork') !== false) {
            $device->os->set($this->device->getCapability('device_os'),
                $this->device->getCapability('device_os_version'));
