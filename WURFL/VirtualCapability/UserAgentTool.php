...
class WURFL_VirtualCapability_UserAgentTool
{
    ...
    protected function assignProperties($device)
    {
        ...
        if ($device->os->setContains($device->device_ua, 'Windows CE', 'Windows Mobile') && $device->browser->set(
                'IE Mobile'
            )
        ) {
            return $device;
        ...
        if (strpos($device->device_ua_normalized, 'iPhone') !== false || strpos(
            ...
            if ($device->os->setRegex(
                    $device->device_ua_normalized,
                    '/Mozilla\/[45]\.0 \((iPhone|iPod|iPad);(?: U;)? CPU(?: iPhone|) OS ([\d_]+) like Mac OS X/',
                    'iOS',
                    2
                ) || $device->os->setRegex(
                    $device->device_ua_normalized,
        ...
        if ($device->os->setRegex(
            $device->device_ua,
            '/^Mozilla\/[0-9]\.0 .+?((?:Windows|Linux|PPC|Intel) [a-zA-Z0-9 _\.\-]+).+bdbrowser(?:_i18n)?\/([\d\.]+)/',
            1
        )
        ) {
        ...
        if (strpos($device->device_ua, 'MSIE') !== false) {
            if ($device->os->setRegex(
                $device->device_ua,
                '/^Mozilla\/[0-9]\.0 \(compatible; MSIE ([\d\.]+); ((?:Windows NT [0-9]+\.[0-9])|(?:Windows [0-9]\.[0-9])|(?:Windows [0-9]+)|(?:Mac_PowerPC))/',
                2
            )
            ) {
        ...
        if (strpos($device->device_ua, 'Trident') !== false || strpos($device->device_ua, ' Edge/') !== false) {
            ...
            if ($device->os->setRegex(
                $device->device_ua,
                '#^Mozilla/[45]\.0 \((Windows NT [\d\.]+);.+Trident.+; rv:([\d\.]+)\.[0-9]+#',
                1
            )
            ) {
            ...
            if ($device->os->setRegex(
                $device->device_ua,
                '#^Mozilla/[45]\.0 \((Windows NT [\d\.]+).+? Edge/([\d\.]+)#',
                1
            )
            ) {
        ...
        if (strpos($device->device_ua, 'YaBrowser') !== false && $device->os->setRegex(
                $device->device_ua,
                '/^Mozilla\/[45]\.[0-9] \((?:Macintosh; )?([a-zA-Z0-9\._ ]+)\) AppleWebKit.+YaBrowser\/([\d\.]+)/',
                1
            )
        ) {
        ...
        if (strpos($device->device_ua, 'OPR') !== false && $device->os->setRegex(
                $device->device_ua,
                '/^Mozilla\/[0-9]\.0 .+?((?:Windows|Linux|PPC|Intel) [a-zA-Z0-9 _\.\-]+).+Chrome\/.+OPR\/([\d\.]+)/',
                1
            )
        ) {
        ...
        if (strpos($device->device_ua, 'Chrome') !== false) {
            ...
            if ($device->os->setRegex(
                $device->device_ua,
                '/^Mozilla\/[0-9]\.0 \(Macintosh;(?: U;)?([a-zA-Z_ \.0-9]+)(?:;)?.+? Chrome\/([\d\.]+)\.?/',
                1
            )
            ) {
            ...
            if ($device->os->setRegex(
                $device->device_ua,
                '/^Mozilla\/[0-9]\.0 \((?:Windows;|X11;)?(?: U; )?([a-zA-Z_ \.0-9]+)(?:;)?.+? Chrome\/([\d\.]+)\.?/',
                1
            )
            ) {
                $device->browser->set('Chrome', $device->os->getLastRegexMatch(2));
        ...
        if (strpos($device->device_ua, 'Firefox') !== false) {
            ...
            if ($device->os->setRegex(
                $device->device_ua,
                '/^Mozilla\/[0-9]\.0 .+(Windows [0-9A-Za-z \.]+;).+?rv:.+?Firefox\/([\d\.]+)/',
                1
            )
            ) {
        ...
        if (strpos($device->browser_ua, 'CFNetwork') !== false) {
            $device->os->set(
                $this->device->getCapability('device_os'),
